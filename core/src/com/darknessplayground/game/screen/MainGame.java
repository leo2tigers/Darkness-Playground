...
public class MainGame implements Screen {
	...
	private void setupMap() {
		...
		this.map.addSpawnPoint(new SpawnPoint(MonsterType.OwO_RANGER, 1000, 100, 0.3));
	...
	public void resize(int width, int height) {
		/* Game Window cannot be resized, so left this blank. */}
	...
	private void handleInput(float dt) {
		if (Gdx.input.isKeyJustPressed(Keys.ESCAPE)) {
			...
			this.isPausing = true;
			/*
			 * this.bgm.stop(); this.dispose(); this.game.gameOver(this.player.getXp());
			 */
		}
		if (Gdx.input.isKeyJustPressed(Keys.UP)) {
			...
			if (this.player.getCurrentTile() != null)
				if (this.player.getCurrentTile().type != TileType.FLOOR)
					this.player.jump_down();
		}

		if (!Gdx.input.isKeyPressed(Keys.LEFT) && !Gdx.input.isKeyPressed(Keys.RIGHT)
				&& this.player.getShootingAnimationDelay() <= 0) {
	...
	private void renderPausedScreen(float dt) {
		this.game.batch.draw(pauseScreen, Gdx.graphics.getWidth() / 2 - this.pauseScreen.getWidth() / 2,
				Gdx.graphics.getHeight() / 2 - this.pauseScreen.getHeight() / 2);
		handlePausedInput();
		if (this.isOnPausedResumeButton()) {
			this.game.batch.draw(pauseScreenResumeBtnActive,
					Gdx.graphics.getWidth() / 2 - this.pauseScreenResumeBtnActive.getWidth() / 2,
					PAUSED_RESUME_BUTTON_Y);
		} else {
			this.game.batch.draw(pauseScreenResumeBtnInactive,
					Gdx.graphics.getWidth() / 2 - this.pauseScreenResumeBtnActive.getWidth() / 2,
					PAUSED_RESUME_BUTTON_Y);
		}
		if (this.isOnPausedExitButton()) {
			this.game.batch.draw(pauseScreenExitBtnActive,
					Gdx.graphics.getWidth() / 2 - this.pauseScreenExitBtnActive.getWidth() / 2, PAUSED_EXIT_BUTTON_Y);
		} else {
			this.game.batch.draw(pauseScreenExitBtnInactive,
					Gdx.graphics.getWidth() / 2 - this.pauseScreenExitBtnActive.getWidth() / 2, PAUSED_EXIT_BUTTON_Y);
		}
	}
	...
	private boolean isOnPausedResumeButton() {
		return (Gdx.input.getX() >= Gdx.graphics.getWidth() / 2 - this.pauseScreenResumeBtnActive.getWidth() / 2
				...
				&& Gdx.input.getY() >= Gdx.graphics.getHeight() - PAUSED_RESUME_BUTTON_Y
						- this.pauseScreenResumeBtnActive.getHeight()
				&& Gdx.input.getY() <= Gdx.graphics.getHeight() - PAUSED_RESUME_BUTTON_Y);
	}
	...
	private boolean isOnPausedExitButton() {
		return (Gdx.input.getX() >= Gdx.graphics.getWidth() / 2 - this.pauseScreenExitBtnActive.getWidth() / 2
				...
				&& Gdx.input.getY() >= Gdx.graphics.getHeight() - PAUSED_EXIT_BUTTON_Y
						- this.pauseScreenExitBtnActive.getHeight()
				&& Gdx.input.getY() <= Gdx.graphics.getHeight() - PAUSED_EXIT_BUTTON_Y);
	}
