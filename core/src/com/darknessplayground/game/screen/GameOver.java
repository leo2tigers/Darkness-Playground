...
public class GameOver implements Screen {
	...
	public void render(float dt) {
		...
		this.game.batch.draw(gameOverText, Gdx.graphics.getWidth()/2 - GAME_OVER_TEXT_DISPLAY_WIDTH/2, 470, GAME_OVER_TEXT_DISPLAY_WIDTH, 150);
		this.finalScoreFont.draw(this.game.batch, finalScoreRendering, Gdx.graphics.getWidth()/2 - finalScoreRendering.width/2, 350);
		if(this.stateTime >= 1) {
			if(this.isOnBackToMenuButton()) {
				this.game.batch.draw(backToMenuBtnActive, Gdx.graphics.getWidth()/2 - BACK_TO_MENU_DISPLAY_WIDTH/2, BACK_TO_MENU_POSITION_Y, BACK_TO_MENU_DISPLAY_WIDTH, BACK_TO_MENU_DISPLAY_HEIGHT);
			}
			else {
				this.game.batch.draw(backToMenuBtn, Gdx.graphics.getWidth()/2 - BACK_TO_MENU_DISPLAY_WIDTH/2, BACK_TO_MENU_POSITION_Y, BACK_TO_MENU_DISPLAY_WIDTH, BACK_TO_MENU_DISPLAY_HEIGHT);
			}
		}
		this.game.batch.end();
	...
	private boolean isOnBackToMenuButton()
	{
		...
		return (Gdx.input.getX() >= centerLine - BACK_TO_MENU_DISPLAY_WIDTH/2 && Gdx.input.getX() <= centerLine + BACK_TO_MENU_DISPLAY_WIDTH/2 &&
				Gdx.input.getY() >= screenHeight - BACK_TO_MENU_POSITION_Y - BACK_TO_MENU_DISPLAY_HEIGHT && Gdx.input.getY() <= screenHeight - BACK_TO_MENU_POSITION_Y);
	}
