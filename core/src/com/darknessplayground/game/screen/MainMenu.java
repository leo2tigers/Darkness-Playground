...
public class MainMenu implements Screen {
	...
	public void render(float dt) {
		...
		this.game.batch.draw(this.gameTitle, 190, 450, GAME_TITLE_DISPLAY_WIDTH, GAME_TITLE_DISPLAY_HEIGHT); // change
																												// from
																												// x845
																												// =
																												// x190
		if (this.stateTime >= 1) {
			if (this.isOnPlayBtn()) {
				this.game.batch.draw(playButtonActive, BUTTON_POSITION_X, PLAY_BUTTON_POSITION_Y,
						PLAY_BUTTON_DISPLAY_WIDTH, PLAY_BUTTON_DISPLAY_HEIGHT);
			} else {
				this.game.batch.draw(playButtonInActive, BUTTON_POSITION_X, PLAY_BUTTON_POSITION_Y,
						PLAY_BUTTON_DISPLAY_WIDTH, PLAY_BUTTON_DISPLAY_HEIGHT);
			}

			if (this.isOnExitBtn()) {
				this.game.batch.draw(exitButtonActive, BUTTON_POSITION_X, EXIT_BUTTON_POSITION_Y,
						EXIT_BUTTON_DISPLAY_WIDTH, EXIT_BUTTON_DISPLAY_HEIGHT);
			} else {
				this.game.batch.draw(exitButtonInActive, BUTTON_POSITION_X, EXIT_BUTTON_POSITION_Y,
						EXIT_BUTTON_DISPLAY_WIDTH, EXIT_BUTTON_DISPLAY_HEIGHT);
			}
	...
	private boolean isOnPlayBtn() {
		if (Gdx.input.getX() >= BUTTON_POSITION_X && Gdx.input.getX() <= BUTTON_POSITION_X + PLAY_BUTTON_DISPLAY_WIDTH
