	...
	public void render(Player player, SpriteBatch batch) {
		{
			...
			try {
				if (player.gun.isReloading()) {
					...
					GlyphLayout ammoDisplay = new GlyphLayout(font, ammo + "/" + maxAmmo, new Color(0, 1, 1, 1), 50,
							Align.left, false);
					this.font.draw(batch, ammoDisplay, Gdx.graphics.getWidth() - 70 - ammoDisplay.width, 10 + 40);
				} else if (ammo <= 0) {
					batch.draw(noAmmoUI, positionX, positionY);
					GlyphLayout ammoDisplay = new GlyphLayout(font, ammo + "/" + maxAmmo, Color.RED, 50, Align.left,
							false);
					this.font.draw(batch, ammoDisplay, Gdx.graphics.getWidth() - 70 - ammoDisplay.width, 10 + 40);
				} else if (!player.gun.isReloading() && ammo > 0) {
					batch.draw(normalUI, positionX, positionY);
					GlyphLayout ammoDisplay = new GlyphLayout(font, ammo + "/" + maxAmmo,
