...
public class Projectile extends GameObject {
	...
	public Projectile(double positionX, double positionY, double width, double height, double orientation, double speed,
	...
	public Projectile(double positionX, double positionY, double width, double height, double orientation, double speed,
			int lifetime, int damage, int damage_check_type, String imgPath, String hitPath) {
		super(positionX, positionY);
		this.orientation = orientation;
		this.speed = speed;
		this.lifetime = lifetime;
	...
	public void render(SpriteBatch batch) {
		try {
			batch.draw(img, (float) positionX, (float) positionY, (float) damageBox.getWidth(),
					(float) damageBox.getHeight());
	...
	public String toString() {
		return "Projectile ( " + (lifetime + this.createDate.getTime() - (new Date()).getTime()) + " ) "
				+ ((this.damageCheckType == 0) ? "TO_MONSTER" : "TO_PLAYER") + " ( " + this.positionX + " , "
				+ this.positionY + " )";
	}
