...
public abstract class Creature extends GameObject {
	...
	/**
	 * This method is used to translate the creature and its components by create
	 * new movementBox and check whether it overlap any tile. If there is any
	 * overlapped tile, the new position will be on that tile.
	 * 
	 * @param x
	 * @param y
	 * @return
	...
	protected double[] translate(double x, double y) {
		...
		URect check_movementBox = new URect(movementBox.getX() + x, movementBox.getY() + y, movementBox.getWidth(),
				movementBox.getHeight(), Color.BLUE);

		Tile check_tile = overlapTile(check_movementBox);
		if (check_movementBox.overlap(check_tile) && Math.abs(check_tile.getX() + check_tile.getWidth() / 2
				- this.positionX - this.movementBox.getWidth() / 2) <= check_tile.getWidth() / 2) {
	...
	/**
	 * This method is used to begin an Attack Event if the creature is alive and
	 * attackable.
	 */
	public void attack() {
